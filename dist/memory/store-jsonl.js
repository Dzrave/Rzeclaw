...
export function getPrivacyIsolatedDir(workspaceDir) {
    return path.join(workspaceDir, ".rzeclaw", "privacy_isolated");
}
/** WO-1511: 某会话的隐私隔离存储文件路径 */
export function getPrivacyIsolatedFilePath(workspaceDir, sessionId) {
...
export class JsonlMemoryStore {
    ...
    async query_by_condition(conditions) {
        let raw;
        try {
        ...
        let entries = lines.map(parseLine).filter((e) => e != null);
        ...
        if (conditions.task_hint != null) {
            ...
            entries = entries.filter((e) => e.task_hint != null && e.task_hint.toLowerCase().includes(kw));
        }
    ...
    async update_validity(id, validity) {
        let raw;
        try {
            raw = await readFile(this.filePath, "utf-8");
        }
...
export function createStore(workspaceDir, workspaceId) {
...
export function createColdStore(workspaceDir, workspaceId) {
...
export function createPrivacyIsolatedStore(workspaceDir, sessionId) {
