...
function taskRelevanceScore(entry, queryTokens, taskHintTokens) {
    ...
    for (const t of allQuery) {
        if (allEntry.has(t))
            overlap += 1;
        else if ([...allEntry].some((e) => e.includes(t) || t.includes(e)))
            overlap += 0.5;
    }
    return overlap;
}
export async function retrieve(store, query, options = {}) {
 ...
 */
export function formatAsCitedBlocks(entries) {
    if (entries.length === 0)
        return "";
    return entries
        .map((e) => `[Memory#${e.id}] (来自 session ${e.provenance.session_id}${e.provenance.turn_index != null ? ` 第 ${e.provenance.turn_index} 轮` : ""}) ${e.content}`)
        .join("\n\n");
}
export const MEMORY_SYSTEM_INSTRUCTION = "以下为来自长期记忆的可靠内容，请仅基于此作答；若引用某条，请标明其 Memory#id。若记忆中无相关信息，请明确说明。";
