...
const TASK_KEYWORDS: Array<{ keywords: string[]; hint: string }> = [
 ...
 */
export function extractTaskHint(userMessage: string): string {
  const raw = (userMessage ?? "").trim();
  if (!raw) return "";

  const normalized = normalize(raw);
  for (const { keywords, hint } of TASK_KEYWORDS) {
    if (keywords.some((kw) => normalized.includes(normalize(kw)))) return hint;
  }
