...
import type { MemoryEntry, MemoryEntryInsert } from "./types.js";
...
function isDuplicate(newContent: string, existingL2: MemoryEntry[]): boolean {
  const norm = normalizeContent(newContent);
  for (const e of existingL2) {
    if (normalizeContent(e.content) === norm) return true;
    if (e.content.length > 0 && norm.length > 0) {
      ...
      if (a > 0 && overlap / Math.max(a, b) >= 0.9) return true;
    }
  }
  return false;
}
 ...
 */
export async function promoteL1ToL2(
  store: IMemoryStore,
  options: PromoteL1ToL2Options
